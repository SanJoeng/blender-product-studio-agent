...
export async function dispatchTool(studio: Studio, projectId: string, runId: string, name: string, args: unknown): Promise<CallToolResult> {
  ...
  switch (name) {
    case 'get_project': {
      ...
      value = { projectId, projectRoot: studio.store.projectDir(projectId), name: p.name, brief: p.brief, notes: p.notes, workingRevisionId: p.workingRevisionId, approvedRevisionId: p.approvedRevisionId, inputs: p.inputs, revisions: p.revisions.slice(-30), renders: p.renders.slice(-12), assets: p.assets, jobs: p.jobs.slice(-6).map(({ result, ...j }) => j) }; break;
    }
    case 'read_skill': {
    ...
    case 'edit_scene': value = studio.edit(projectId, data, runId); break;
